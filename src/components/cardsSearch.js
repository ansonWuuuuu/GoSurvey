...
import { POSTS_QUERY, USER_QUERY, SURVEY_QUERY, SURVEY_NAME_QUERY } from "../graphql/queries"
...
function CardsSearch() {
    const { ID, filterType, answer_query, clickedId, setClickedId, survey_query, addFavorite_mut, search, setSearch} = useSurvey();
    ...
    // console.log("reQuery")
    const { data, loading, error, subscribeToMore } = useQuery(SURVEY_NAME_QUERY,{
        variables: {
            surveyName: search
        }
    })
